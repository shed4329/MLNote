...
def read_from_data(file_path="data2.txt"):
    ...
    with open(file_path, 'r') as file:
        for line in file:
            line = line.strip()
            parts = line.split()
            X.append([int(parts[0]), int(parts[1]), int(parts[2]), int(parts[3])])
...
if __name__ == '__main__':
    ...
    plt.figtext(0.5, 0.01,
                f"Model: {model.coef_[0, 0]:.4f}x1 + {model.coef_[0, 1]:.4f}x1² + {model.coef_[0, 2]:.4f}x2 + {model.coef_[0, 3]:.4f}x2² + {model.intercept_[0]:.4f} = 0",
